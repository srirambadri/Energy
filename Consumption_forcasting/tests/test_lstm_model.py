...
@pytest.fixture
def sample_data_lstm():
    # Generate some synthetic data for testing LSTM
...
def test_lstm_forecaster_train_predict(sample_data_lstm):
    X_raw, y_raw = sample_data_lstm
    if len(X_raw) < 60: 
    ...
    forecaster = LSTMForecaster(sequence_length=10, epochs=2) 
...
def test_lstm_forecaster_evaluate(sample_data_lstm):
    X_raw, y_raw = sample_data_lstm
    if len(X_raw) < 60:
        pytest.skip("Not enough data for sequence creation and train-test split.")
...
def test_lstm_forecaster_predict_before_train(sample_data_lstm):
    X_test_raw, y_test_raw = sample_data_lstm[0].iloc[80:], sample_data_lstm[1].iloc[80:]
    forecaster = LSTMForecaster(sequence_length=10)
